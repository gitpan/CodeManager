...
#	'(\'.*?\')',				{ color => 0xff0066,},
	"('([^'\\\\]++|\\\\.)*+')",	{ color => 0xaa0000,},
	'(;)',						{ color => 0x000000,	style => fs::Bold,},
	'(\b(FOR\s+\w+\s+IN|WHILE|LOOP|END\s+LOOP)\b)',
								{ color => 0x7700ff,},
	'(\b(begin\s+transaction\s*;|commit;|rollback;|begin|end|declare|returns))',
								{ color => 0x0000ff,	style => fs::Underlined,},
	'(\b(NULL|return|EXIT)\b)',	{ color => 0xff0000,	backColor => 0xffffdd,},
	'(\b(and|or|if|when|elseif|elsif|end\s+if\s*;|found|not found|then|else)\b)',
								{ color => 0x0000ff,},
